...
class JwtTokenUtilTest {
    ...
    void setUp() {
        ...
        // 模拟Redis黑名单检查（仅部分用例会触发，使用 lenient 避免严格存根校验误报）
    ...
    void testGenerateToken() {
        ...
        // 生产端统一使用 roleNames/permissionCodes 声明键（与过滤器、登录、AuthController 一致）
        assertEquals(roles, claims.get("roleNames", List.class));
        assertEquals(permissions, claims.get("permissionCodes", List.class));
    }
    ...
    void testExtractRoles() {
        ...
        // 从Token中提取角色（生产端声明键为 roleNames）
        List<String> extractedRoles = jwtTokenUtil.extractRoleNames(token);
    ...
    void testExtractPermissions() {
        ...
        // 从Token中提取权限（生产端声明键为 permissionCodes）
        List<String> extractedPermissions = jwtTokenUtil.extractPermissionCodes(token);

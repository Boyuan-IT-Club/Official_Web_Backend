...
import club.boyuan.official.domain.interview.scheduling.ClassroomAssigner;
import club.boyuan.official.domain.interview.scheduling.InterviewSlotTimeCalculator;
import club.boyuan.official.domain.interview.scheduling.InterviewUrgencyScorer;
import club.boyuan.official.domain.interview.service.IInterviewAssignmentService;
...
public class InterviewAssignmentServiceImpl implements IInterviewAssignmentService {
    ...
    private Map<String, Integer> calculateTimeSlotDemand(Map<Integer, List<String>> userPreferredTimes) {
        Map<String, Integer> demand = InterviewSlotTimeCalculator.calculateTimeSlotDemand(userPreferredTimes);
    ...
    private double calculateUrgencyScore(CandidateInfo candidate,
                                         ...
                                         Map<String, Map<LocalDateTime, Boolean>> departmentSlotAvailability) {
        return InterviewUrgencyScorer.calculateUrgencyScore(
                candidate.preferredTimes, candidate.firstDepartment, timeSlotDemand, departmentSlotAvailability);
    }
    ...
    private List<LocalDateTime> generateTimeSlotsForSingleDay(LocalDate day) {
        return InterviewSlotTimeCalculator.generateTimeSlotsForSingleDay(day);
    }
    ...
    private List<LocalDateTime> generateTimeSlots(LocalDate startDate, LocalDate endDate) {
        return InterviewSlotTimeCalculator.generateTimeSlots(startDate, endDate);
    }
        ...
        for (String preferredTime : preferredTimes) {
            ...
            if (hasAvailableSlotAndClassroom(preferredTime, department, departmentSlotAvailability, classroomAssigner)) {
                ...
                if (assignedSlot != null) {
                    ...
                    if (classroom != null) {
                        // 成功分配时间和教室
                        String period = InterviewSlotTimeCalculator.periodOf(assignedSlot.toLocalTime());
    ...
    private boolean isSlotMatchPreference(LocalDateTime slotTime, String preferredTime) {
        return InterviewSlotTimeCalculator.isSlotMatchPreference(slotTime, preferredTime);
    }
